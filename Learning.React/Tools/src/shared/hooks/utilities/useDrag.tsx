 ...
 } : IUseDragParameters) : IUseDragResponse<T> =>
{
  ...
  const startDragging = useCallback(() => { setIsDragging(true); onDragStart?.(); }, []);
  const dragging = useCallback((event: Event) => { event.preventDefault(); onDragging?.(); }, []);
  const stopDragging = useCallback(() => { setIsDragging(false); onDragEnd?.(); }, []);
