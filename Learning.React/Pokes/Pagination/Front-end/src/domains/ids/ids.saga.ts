...
import {
	getIdsFailedAction,
	getIdsSuccessAction, getPaginateIdsFailedAction, GetPaginateIdsRequestAction, getPaginateIdsSuccessAction,
	IdAction
} from "./ids.action";
import { getIds, getPaginateIds} from "./ids.api";
...
export function* getPaginateIdsSaga(action: GetPaginateIdsRequestAction) {
	try {
		...
		const paginateResponse: IPaginateResponse<number> = yield call(getPaginateIds, page, pageSize);
