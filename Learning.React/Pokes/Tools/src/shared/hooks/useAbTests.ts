...
import {abTestReducerKey, abTestSagaKey} from "../domains/abTest/abTest.configuration";
...
export const useAbTests = (...abTestsIds : number[]) : boolean[] =>
{
	const abTestsState = useSelector(selectAbTestsState)
	const dispatch = useDispatch()
	
	const reducerIsInjected = useLazyReducer(abTestReducerKey, 'shared/domains/abTest/abTest.reducer')
	const sagaIsInjected = useLazySaga(abTestSagaKey, 'shared/domains/abTest/abTest.saga')
	
	const [isInitialized, setIsInitialized] = useState(false)
	
	useEffect(() =>
	{
		if (reducerIsInjected && sagaIsInjected && !abTestsState.initialized && !isInitialized)
		{
			dispatch(getAbTestsAction())
			setIsInitialized(true)
		}
	}, [reducerIsInjected, sagaIsInjected, abTestsState.initialized, isInitialized, dispatch])
